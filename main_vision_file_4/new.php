...
if($is_result != 1){
    $queryQ = "SELECT * FROM `registeration` ORDER BY `registeration`.`reg_publish_date` DESC";
}
if(isset($_SESSION['user_id'])){
    $user_id = $_SESSION['user_id'];
  $queryQ = "SELECT * FROM `registeration` WHERE NOT `reg_user_id`= $user_id  ORDER BY `reg_publish_date` DESC";
 }
